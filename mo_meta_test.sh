...
for task in ${tasks[@]}
do
    CUDA_VISIBLE_DEVICES=${gpu} python -u meta_test.py \
        -seed ${seed} \
        -batch_size ${batch_size} \
        -translate_batch_size ${batch_size} \
        -meta_test_task ${task} \
        -data processed_data/meta-test/${task} \
        -output meta_test_${task}_data_${data_ratio}_inner_${inner_iter}_seed_${seed}.predict \
        ...
        -share_embeddings \
        -replace_unk \
        -fast > nohup_meta_test_${task}_data_${data_ratio}_inner_${inner_iter}_seed_${seed}.log 2>&1
done
