...
def build_dataset(fields, data_type, src_data_iter=None, src_path=None,
    ...
    dataset.fields['graph'] = torchtext.data.Field(sequential = False)
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    if data_type == 'text':
        ...
        if share_vocab:
            ...
            print(merged_vocab.stoi)
            print(merged_vocab.itos)
...
class DatasetLazyIter(object):
    ...
    def __init__(self, datasets, fields, batch_size, batch_size_fn,
                 device, is_train, data_ratio=1.0):
        ...
        # self.data_ratio = data_ratio if is_train else 1.0

        self.cur_iter, num_batch = self._next_dataset_iterator(datasets)
        self.data_size = int(num_batch * data_ratio)
        # for evaluate on 16~256 training samples
        # self.data_size = (data_ratio // batch_size) if is_train else num_batch
        
        logger.info('Train: {}, Total batches: {}/{} with data size: {:.2f} and batch size {}' \
            .format(is_train, self.data_size, num_batch, self.data_size, self.batch_size))
    ...
    def __iter__(self):
        ...
        while self.cur_iter is not None:
            ...
            self.cur_iter, num_batch = self._next_dataset_iterator(dataset_iter)
    ...
    def _next_dataset_iterator(self, dataset_iter):
        ...
        # print(self.cur_dataset.examples[0].src)
        # input()
        # setattr(self.cur_dataset.examples[0], 'graph', 1)
        # print(self.cur_dataset.examples[0].__dict__.keys()) 
        # input()
...
def build_dataset_iter(datasets, fields, opt, is_train=True):
    ...
    return DatasetLazyIter(datasets, fields, batch_size, batch_size_fn,
                           device, is_train, data_ratio=opt.data_ratio)
...
def _load_fields(dataset, data_type, opt, checkpoint):
    if checkpoint is not None:
        ...
        def my_func(batch):
            ...
            g_loader = torch.utils.data.DataLoader(g_dataset, batch_size=len(rawstrs), shuffle=False, collate_fn=myutils.collate_dgl)
