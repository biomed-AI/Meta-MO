    ...
    with open(filename, 'r', encoding='utf-8') as file:
        for line in file:
            if line.strip() == '' or index == 0:
                index = 1
                continue
            mol, score = line.strip().split(',')
            if score == 'Invalid SMILES':
                score = 0.0
            else:
                score = float(score)
...
def check_valid(smiles):
    return False if smiles is None or smiles == '' else Chem.MolFromSmiles(smiles) is not None

def calculate_metrics(task, task_type, data_type, target_mols):
    assert task_type in ['dev', 'test']
    assert data_type in ['dev', 'test']
    src_scores_path = 'data/meta-tgt/' + task + '/scores.txt'
    src_mols_path = 'processed_data/meta-' + task_type + '/' + task + '/src-' + data_type + '.txt'

    scores_dict = {}
    with open(src_scores_path, 'r', encoding='utf-8') as f:
        for line in f:
            mol, score = line.strip().split(' ')
            scores_dict[mol] = float(score)

    src_mols = []
    with open(src_mols_path, 'r', encoding='utf-8') as f:
        for line in f:
            mol = line.strip().replace(' ', '')
            src_mols.append([mol, scores_dict[mol]])
    
    assert 10 * len(src_mols) == len(target_mols)
    sims = []
    uncons_improvs = []
    cons_improvs = []
    success = 0
    ...
    for i in range(len(src_mols)):
        success_flag = False
        weak_success_flag = False
        good = []
        for j in range(10):
            x, x_score = src_mols[i][0], src_mols[i][1]
            y, y_score = target_mols[10 * i + j][0], target_mols[10 * i + j][1]
            valid += 1 if check_valid(y) else 0
            unique_mols.add(y)
            sim = similarity(x, y)
            improv = y_score - x_score
            sims.append(sim)
            uncons_improvs.append(improv)
            if improv >= 1.0:
                weak_success_flag = True
            if sim >= 0.4:
                cons_improvs.append(improv)
                if improv >= 1.0:
                    success_flag = True
                    good.append(y)
        if success_flag:
            success += 1
        if weak_success_flag:
            weak_success += 1
        ...
        for m in range(len(good)):
            for n in range(m + 1, len(good)):
                div += 1 - similarity(good[m], good[n])
                total += 1
        div /= total
        divs.append(div)

    sims = np.array(sims)
    uncons_improvs = np.array(uncons_improvs)
    cons_improvs = np.array(cons_improvs)
    divs = np.array(divs)

    avg_sim, sim_std = np.mean(sims), np.std(sims)
    avg_uncons_improvs, uncons_improv_std = np.mean(uncons_improvs), np.std(uncons_improvs)
    avg_cons_improv, cons_improv_std = np.mean(cons_improvs), np.std(cons_improvs)
    avg_div, div_std = np.mean(divs), np.std(divs)
    success_rate = success / len(src_mols)
    weak_success_rate = weak_success / len(src_mols)
    unique_rate = len(unique_mols) / (10 * len(src_mols))
    validity = valid / (10 * len(src_mols))

    round_digits = 4
    metrics = {
        'similarity': [round(avg_sim, round_digits), round(sim_std, round_digits)],
        'uncons_improvment': [round(avg_uncons_improvs, round_digits), round(uncons_improv_std, round_digits)],
        'cons_improvement': [round(avg_cons_improv, round_digits), round(cons_improv_std, round_digits)],
        'diversity': [round(avg_div, round_digits), round(div_std, round_digits)],
        'success_rate': round(success_rate, round_digits),
        'weak_success_rate': round(weak_success_rate, round_digits),
        'unique_rate': round(unique_rate, round_digits),
        'validity': round(validity, round_digits),
        'n_src_mols': len(src_mols),
        }
