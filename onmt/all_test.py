...
def main(opt, device_id):
    ...
    if not os.path.exists('experiments/all_test'):
        ...
        os.mkdir('experiments/all_test/' + opt.meta_test_task)
    model_saver = build_model_saver(model_opt, 'experiments/all_test/' + opt.meta_test_task + '/model', opt, model, fields, optim)
    ...
    out_file = None
    dummy_parser = argparse.ArgumentParser(description='all_test.py')
    opts.model_opts(dummy_parser)
    dummy_opt = dummy_parser.parse_known_args([])[0]
    for model_path in os.listdir('experiments/all_test/' + opt.meta_test_task):
        if model_path.find('.pt') == -1:
            continue
        if out_file is None:
            out_file = codecs.open(opt.output, 'w', 'utf-8')
        fields, model, model_opt = onmt.model_builder.load_test_model(opt, dummy_opt.__dict__, model_path='experiments/all_test/' + opt.meta_test_task + '/' + model_path)
        ...
        all_scores, all_predictions = translator.translate(
            ...
            attn_debug=False)
        # dump predictions
        f = open('experiments/all_test/' + opt.meta_test_task + '/dev_predictions.csv', 'w', encoding='utf-8')
        ...
        test_path = '\"' + 'experiments/all_test/' + opt.meta_test_task + '/dev_predictions.csv' + '\"'
        checkpoint_path = '\"scorer_ckpts/' + opt.meta_test_task + '/model.pt' + '\"'
        preds_path = '\"' + 'experiments/all_test/' + opt.meta_test_task + '/dev_scores.csv' + '\"'
        ...
        score = read_score_csv('experiments/all_test/' + opt.meta_test_task + '/dev_scores.csv')
    ...
    out_file.close()
    out_file = codecs.open(opt.output, 'w', 'utf-8')
    fields, model, model_opt = onmt.model_builder.load_test_model(opt, dummy_opt.__dict__, model_path='experiments/all_test/' + opt.meta_test_task + '/' + best_model_path)
