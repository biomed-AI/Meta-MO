...
class Translator(object):
    ...
    def translate(self,
        ...
        for i in range(len(data)):
            if i % 500 == 0:
                print(i)
            setattr(data.examples[i], 'graph', myutils.str2graph(data.examples[i].src))
    ...
    def _fast_translate_batch(self,
        ...
        enc_states, memory_bank, src_lengths \
            = self.model.encoder((src,batch.graph), src_lengths)
        dec_states = self.model.decoder.init_decoder_state(
            (src,batch.graph), memory_bank, enc_states, with_cache=True)
    ...
    def _translate_batch(self, batch, data):
        ...
        enc_states, memory_bank, src_lengths \
            = self.model.encoder((src, batch.graph), src_lengths)
        dec_states = self.model.decoder.init_decoder_state(
            (src,batch.graph), memory_bank, enc_states)
    ...
    def _run_target(self, batch, data):
        ...
        enc_states, memory_bank, src_lengths \
            = self.model.encoder((src,batch.graph), src_lengths)
        dec_states = \
            self.model.decoder.init_decoder_state((src,batch.graph), memory_bank, enc_states)
