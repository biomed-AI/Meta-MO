...
def main(opt, device_id):
    ...
    for i in range(0, opt.train_epochs, 10):
        ...
        if not os.path.exists('experiments/all_dev'):
            ...
            os.mkdir('experiments/all_dev/' + opt.meta_dev_task)
        model_saver = build_model_saver(model_opt, 'experiments/all_dev/' + opt.meta_dev_task + '/model', opt, model, fields, optim)
        ...
        for model_path in os.listdir('experiments/all_dev/' + opt.meta_dev_task):
            if model_path.find('.pt') == -1:
                continue
            if out_file is None:
                out_file = codecs.open(opt.output, 'w+', 'utf-8')
            
            fields, model, model_opt = onmt.model_builder.load_test_model(opt, dummy_opt.__dict__, model_path='experiments/all_dev/' + opt.meta_dev_task + '/' + model_path)
            ...
            all_scores, all_predictions = translator.translate(
                ...
                attn_debug=False)
            # dump predictions
            f = open('experiments/all_dev/' + opt.meta_dev_task + '/dev_predictions.csv', 'w', encoding='utf-8')
            ...
            score = read_score_csv('experiments/all_dev/' + opt.meta_dev_task + '/dev_scores.csv')
        ...
        if out_file is None:
            out_file = codecs.open(opt.output, 'w+', 'utf-8')
        fields, model, model_opt = onmt.model_builder.load_test_model(opt, dummy_opt.__dict__, model_path='experiments/all_dev/' + opt.meta_dev_task + '/' + best_model_path)
        ...
        all_scores, all_predictions = translator.translate(
            ...
            attn_debug=False)
        # dump predictions
        f = open('experiments/all_dev/' + opt.meta_dev_task + '/test_predictions.csv', 'w', encoding='utf-8')
        f.write('smiles,property\n')
        for n_best_mols in all_predictions:
            for mol in n_best_mols:
                f.write(mol.replace(' ', '')+',0\n')
        ...
        score = read_score_csv('experiments/all_dev/' + opt.meta_dev_task + '/test_scores.csv')
